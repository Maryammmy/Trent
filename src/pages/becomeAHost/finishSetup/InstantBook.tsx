...
const storedBookingSetting = sessionStorage.getItem("instantBookSelection");
function InstantBook() {
  const { t } = useTranslation();

  const [selectedBookingSetting, setSelectedBookingSetting] = useState<string>(
  storedBookingSetting || ""
  );

  const handleSelect = (title: string) => {
    setSelectedBookingSetting(title);
    sessionStorage.setItem("instantBookSelection", title);
  };
  ...
  return (
    <div className="py-10">
      <div className="hosting-layout flex flex-col justify-center max-w-screen-sm mx-auto px-5 md:px-0 pb-10">
        ...
        <div className="grid grid-cols-1 gap-4">
          {instantBook.map((item, index) => {
            ...
            const isSelected = selectedBookingSetting === title;
      ...
      <BackAndNext
        ...
        isNextDisabled={!selectedBookingSetting}
      />
    </div>
  );
}
